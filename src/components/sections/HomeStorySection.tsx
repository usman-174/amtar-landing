    ...
    <section ref={ref} className="relative px-4 py-16 md:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start">
          <div>
            <p className="story-title text-xs font-semibold uppercase tracking-[0.2em] text-blue-200">{t("horizontal.heading")}</p>
            <h2 className="mt-3 text-balance text-3xl font-semibold text-slate-50 md:text-5xl">{t("horizontal.subtitle")}</h2>
            <p className="mt-5 max-w-xl text-sm leading-relaxed text-slate-200/70 md:text-base">
              {t("home.storyBody", {
                defaultValue:
            ...
            <div className="mt-8 space-y-3">
              {workflowCards.map((card) => (
                <div
                  key={card.title}
                  className="story-title rounded-2xl border border-white/10 bg-slate-800/45 px-4 py-3 text-sm font-medium text-slate-100 shadow-sm"
                >
                  {card.title}
                </div>
              ))}
